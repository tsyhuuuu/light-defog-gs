    ...
    for condition, group in df.groupby("condition"):
        ...
        for metric in metrics:
            if metric in group.columns:
                ...
                stats_row[f"{metric}_std"] = 1.96*group[metric].std()/np.sqrt(len(group))  # 95% CI
...
def create_dynamic_2d_plot(base_dir, dataset, betas, alphas, conditions):
    ...
    for idx, metric in enumerate(metrics):
        ...
        ax.set_xlabel(r"Fog Density Level $β$ [-]")
    ...
    alpha_slider = Slider(ax_slider, r"Brightness Level $A$ [-]", min(df["alpha"]), max(df["alpha"]),
    ...
    for row_idx, condition in enumerate(conditions):
        for col_idx, metric in enumerate(metrics):
            ...
            ax.set_title(title_text, fontsize=12, fontweight='bold' if is_best else 'normal', 
                        pad=10)
            ax.set_xlabel(r"Fog Density Level $β$ [-]", fontsize=11, fontweight='bold')
            ax.set_ylabel(r"Brightness Level $A$ [-]", fontsize=11, fontweight='bold')
...
def create_heatmap_plot(base_dir, dataset, betas, alphas, conditions):
    ...
    condition_labels = {
        ...
        "defogged_gs-w": "GS-W",            # "Defogged (WildGaussians)" 
    ...
    for row_idx, condition in enumerate(conditions):
        for col_idx, metric in enumerate(metrics):
            ...
            ax.set_xlabel(r"Fog Density Level $β$ [-]", fontsize=16.5)
            ax.set_ylabel(r"Brightness Level $A$ [-]", fontsize=16.5)

            # Colorbar
            cbar = plt.colorbar(im, ax=ax)
            cbar.set_label(f"{metric} [dB]", rotation=270, labelpad=20, fontsize=17.5) if metric == "PSNR" else cbar.set_label(f"{metric} [-]", rotation=270, labelpad=20, fontsize=17.5)
...
def create_3d_plot(base_dir, dataset, betas, alphas, conditions):
    ...
    for idx, metric in enumerate(metrics, 1):
        ...
        ax.set_xlabel(r"Fog Density Level $β$ [-]")
        ax.set_ylabel(r"Brightness Level $A$ [-]")
        ax.set_zlabel(f"metric [dB]") if metric == "PSNR" else ax.set_zlabel(f"metric [-]")
...
Examples:
  ...
  python metrics.py --plot-type 2d --betas 1 2 3 4 --alpha-step 50
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        default=["defogged_clahe", "defogged_gs-w", "defogged"],
